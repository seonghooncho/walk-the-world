...
public class PostService {
  ...
  public ApiResponse<List<PostResponse>> getPosts(
      Long userId, String cityId, String filter, String cursorToken, int limit) {
    ...
    List<PostResponse> responses = posts.stream().map(p -> buildPostResponse(p, userId)).toList();
  ...
  public ApiResponse<List<CommentResponse>> getComments(
      Long postId, String cursorToken, int limit) {
    PageRequest pageable = PageRequest.of(0, limit + 1);
    ...
    List<CommentResponse> responses = comments.stream().map(this::buildCommentResponse).toList();
