...
public class CurrencyService {
  ...
  public ApiResponse<List<CurrencyTransactionResponse>> getTransactions(
      Long userId, String type, String cursorToken, int limit) {
    ...
    if (cursorToken != null && !cursorToken.isBlank()) {
      ...
      if (type != null && !type.isBlank()) {
        ...
        txns =
            transactionRepository.findByUserIdAndTypeCursor(
                userId, ct, c.createdAt(), c.id(), pageable);
      } else {
    ...
    List<CurrencyTransactionResponse> responses =
        txns.stream()
            .map(
                t ->
                    CurrencyTransactionResponse.builder()
                        .id(t.getId())
