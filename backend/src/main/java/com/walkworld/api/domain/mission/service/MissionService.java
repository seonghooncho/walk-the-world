...
public class MissionService {
  ...
  public Map<String, List<MissionResponse>> getMissions(
      Long userId, String cityId, String statusFilter) {
    ...
    Map<String, UserMission> userMissionMap =
        userMissionRepository.findByUserId(userId).stream()
  ...
  public MissionCompleteResponse completeMission(
      Long userId, String missionId, String imageKey, String text, boolean autoPost) {
    Mission mission =
        missionRepository
            .findById(missionId)
    ...
    if (mission.getReward() != null && !mission.getReward().isBlank()) {
      badgeService.awardBadge(
          userId, missionId, mission.getCityId(), mission.getReward(), mission.getEmoji());
    }
    ...
    if (autoPost) {
      ...
      Post post =
          Post.builder().userId(userId).cityId(mission.getCityId()).content(content).build();
  ...
  public MissionStatsResponse getStats(Long userId) {
    ...
    Map<String, List<Mission>> byCityId =
        allMissions.stream()
            .collect(
                Collectors.groupingBy(Mission::getCityId, LinkedHashMap::new, Collectors.toList()));
    ...
    for (var entry : byCityId.entrySet()) {
      ...
      cityStats.put(
          entry.getKey(),
          MissionStatsResponse.CityStats.builder()
              .completed(completed)
