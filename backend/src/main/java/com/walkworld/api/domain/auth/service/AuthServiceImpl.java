...
public class AuthServiceImpl implements AuthService {
    ...
    public TokenResDTO signup(SignupReqDTO request) {
        ...
        currencyRepository.save(UserCurrency.builder()
                .userId(user.getId())
                .coupons(2)
                .hearts(5)
                .build());
    ...
    private KakaoUserInfo fetchKakaoUserInfo(String accessToken) {
        try {
            ...
            if (body == null) throw new AuthException(AuthErrorCode.KAKAO_AUTH_FAILED);
    ...
    private GoogleUserInfo verifyGoogleIdToken(String idToken) {
        try {
            ...
            Map body = response.getBody();
            if (body == null) throw new AuthException(AuthErrorCode.GOOGLE_AUTH_FAILED);
    ...
    private TokenResDTO generateTokens(Long userId) {
        ...
        refreshTokenRepository.save(RefreshToken.builder()
                .userId(userId)
                .token(refreshToken)
                .expiresAt(LocalDateTime.now().plusMillis(jwtProvider.getRefreshTokenExpiry()))
                .build());
