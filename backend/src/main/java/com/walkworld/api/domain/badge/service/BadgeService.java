...
public class BadgeService {
  ...
  public BadgeListResponse getBadges(Long userId, String cityId, Boolean earnedFilter) {
    List<Mission> missions =
        missionRepository.findAllByOrderByCityIdAscSortOrderAsc().stream()
            .filter(m -> m.getReward() != null && !m.getReward().isBlank())
            .toList();

    Map<String, UserBadge> earnedMap =
        badgeRepository.findByUserId(userId).stream()
            .collect(Collectors.toMap(UserBadge::getMissionId, b -> b));

    Map<String, City> cityMap =
        cityRepository.findAll().stream().collect(Collectors.toMap(City::getId, c -> c));
  ...
  public BadgeStatsResponse getStats(Long userId) {
    ...
    Map<String, List<BadgeResponse>> byCityId =
        all.getBadges().stream()
            .collect(
                Collectors.groupingBy(
                    BadgeResponse::getCityId, LinkedHashMap::new, Collectors.toList()));

    for (var entry : byCityId.entrySet()) {
      int earned = (int) entry.getValue().stream().filter(BadgeResponse::getEarned).count();
      cityStats.put(
          entry.getKey(),
          BadgeStatsResponse.CityBadgeStats.builder()
              .earned(earned)
