...
| 채팅 경계 | 공백 전송 불가, 500자 메시지 허용, 재진입 후 메시지 유지 | Playwright | 자동화 |
| 댓글 | 게시글 상세에서 댓글 작성 후 목록에 표시 | Playwright | 자동화 |
| 입력 경계 | 댓글/게시글 500자 입력 제한과 501자 차단 | Playwright | 자동화 |
| 프로필 시트 | 닫힌 내 게시물 시트가 불필요한 내 게시물 조회로 프로필 렌더를 깨뜨리지 않음 | Playwright | 자동화 |
| 회원탈퇴 | 탈퇴 후 토큰 삭제와 로그인 복귀, 30일 내 복구 안내 | Playwright, backend test | 자동화 |
| 실서비스 QA | 운영 URL에서 공개/인증/입력/친구/채팅/탈퇴 경계 검증 | Playwright live | 자동화 |
